...
}elseif($_REQUEST['tipo']==5) //manzanas
{
	...
	if(count($datos)>0)
	{
		foreach($datos as $d)
		{
			...
			$data=str_replace(",","|",$data);
			$data=str_replace(" ",",",$data);
			$texto="<strong>DATOS MANZANA</strong>";
			$texto.="<br><br>Total Hogares: ".$d[7]."";
			$texto.="<br>Estrato social: ".$d[8]."";
			$texto.="<br>Hombres: ".$d[12]."";
			$texto.="<br>Mujeres: ".$d[13]."";
			$texto.="<br>Total Personas: ".$d[10]."";
			...
			<?PHP
			}elseif(strtolower($d[8])=="d")
			{
				?>
				opt_style=CM_myStyles2;
			<?PHP
			}elseif(strtolower($d[8])=="c2")
			{
				?>
				opt_style=CM_myStyles3;
			<?PHP
			}elseif(strtolower($d[8])=="c3")
			{
				?>
				opt_style=CM_myStyles4;
			<?PHP
			}elseif(strtolower($d[8])=="abc1")
			{
				?>
