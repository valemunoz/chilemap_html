		...
		if(getEstadoServicio(CM_id)==1)
		{
			if(CM_id_servicio_gis== CM_id_antena)
			{
				if(map.getZoom()  >= MIN_ZOOM_ANTENAS)
				{
					...
					{
						mensaje("El servicio de antenas solo esta disponible desde un zoom "+MIN_ZOOM_ANTENAS+". Si desea acercarse a ese zoom haga click <a href=javascript:map.zoomTo("+MIN_ZOOM_ANTENAS+");>aqui</a>","myPopup");
					}
			}else
			{
				activarServicio(CM_id);
				...
				if(CM_servicios_estado[i]==0)
				{
					if(CM_servicios[i]== CM_id_antena)
					{
						if(map.getZoom()  >= MIN_ZOOM_ANTENAS)
						{
							...
							{
								deleteServicioMapa(i);
								mensaje("<p>El servicio de antenas solo esta disponible desde un zoom "+MIN_ZOOM_ANTENAS+". Si desea acercarse a ese zoom haga click <a href=javascript:map.zoomTo("+MIN_ZOOM_ANTENAS+");>aqui</a></p>","myPopup");
							}
					}else
					{
...
function loadTransantiagoPar()
{
	...
	var AM_exten = getExtencion();
		$("#resul_otros").load("query_mapa.php", 
