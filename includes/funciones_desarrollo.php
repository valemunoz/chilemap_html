...
function getServicioPagoXId($id)
{
	...
	$sql="select id_servicio,nombre_servicio,categoria,latitud,longitud,calle,numero_municipal,comuna,descripcion,id_categoria from cm_servicios where id_servicio=".$id." and estado=2 and fecha_inicio <= '".$fecha."' and fecha_termino >= '".$fecha."'";
	$rsCalle = pg_query($dbPg, $sql);	
	//echo $sql;
	while ($rowCalle = pg_fetch_row($rsCalle)){		
		//$arr_callesComuna[$rowCalle[0]] = $rowCalle[1];
...
function getServiciosXRadioPagos($servicios,$limite,$lat,$lon,$radio)
{
  ...
  ST_GeographyFromText('POINT(".$lon." ".$lat.")'), 
  ST_GeographyFromText(st_AsText(geom))
  )/1000 as radio,id_categoria,descripcion from cm_servicios where estado=2 and fecha_inicio <= '".$fecha."' and fecha_termino >= '".$fecha."' and query_completa ilike '%".$servicios."%' and ST_Distance(
  ST_GeographyFromText('POINT(".$lon." ".$lat.")'), 
  ST_GeographyFromText(st_AsText(geom))
  )/1000 < ".$radio."";
	$sql .=" order by radio";
...
function getManzanasExtend($latI,$latS,$lonD,$lonI)
{
  ...
  //$sql2 = "SELECT manzent,es_e,es_d,es_c3,es_c2,es_abc1,total_hogar,nivel_se,st_astext(geom),personas,region FROM manzana_chile where ST_Contains(ST_GeomFromText('POLYGON((".$lonI." ".$latI.", ".$lonI." ".$latS.", ".$lonD." ".$latS.", ".$lonD." ".$latI.",".$lonI." ".$latI."))',4326),ST_Centroid(geom))";		
  $rs2 = pg_query($dbPg, $sql2);
...
function getServicioExtendPagosFull($latI,$latS,$lonD,$lonI)
{
	$dbPg=pgSql_db();
	$fecha=date("Y-m-d H:i:s");
	$sql2 = "SELECT id_categoria,latitud,longitud,calle,numero_municipal,comuna,region,id_comuna,id_region,categoria,nombre_servicio,descripcion FROM cm_servicios where estado=2 and fecha_inicio <= '".$fecha."' and fecha_termino >= '".$fecha."' and latitud<=".$latS." and latitud >=".$latI." and longitud <=".$lonD." and longitud >=".$lonI."";		
  $rs2 = pg_query($dbPg, $sql2);
	//echo "<br>".$sql2;
	while ($row2 = pg_fetch_row($rs2))
		{
