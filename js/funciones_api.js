...
var CM_vectorLineas,popup_mini;
...
{
	 CM_servicios_pagos[CM_id]=CM_id_serv; //id original servicios BD
	 CM_servicios_pagos_estado[CM_id]=1; //inactivo
	 CM_servicios_pagos_icono[CM_id]=CM_icono; //inactivo
	 markers_ondemandPagos[CM_id] = new OpenLayers.Layer.Markers( ""+CM_nombre+"" );
   map.addLayer(markers_ondemandPagos[CM_id]);
   markers_ondemandPagos[CM_id].setZIndex(zindex_marcador);
...
function activarServicioPago(CM_id)
{
	 CM_servicios_pagos_estado[CM_id]=0;
}
...
function desactivarServicioPago(CM_id)
{
	var elm=document.getElementById("img_serv_pago_"+CM_id);
	elm.className="serv_circulo_off";
	 CM_servicios_pagos_estado[CM_id]=1;
}
function getEstadoServicio(CM_id)
{
	 return CM_servicios_estado[CM_id];
}
function getEstadoServicioPago(CM_id)
{
	 return CM_servicios_pagos_estado[CM_id];
}
function loadMovimiento()
{
	...
	if(CM_manzana==true)
	{
		if(map.getZoom()>=MIN_ZOOM_MANZANA)
		{
		...
		}else
			{
				deletePoligonos();
			 CM_manzana=false;
			 var elm=document.getElementById("img_serv_002");
			elm.className="serv_circulo_off";
				showAlerta("Para poder desplegar las manzanas debes acercarte mas al mapa. El zoom minimo es "+MIN_ZOOM_MANZANA+".",4000);
			}
			markers.setZIndex(zindex_marcador_direccion);
			markers_servicio.setZIndex(zindex_marcador);
	}
	...
	if(CM_transantiago==true)
	{
		if(map.getZoom() >= MIN_ZOOM_TRANS)
		{
			...
			setTimeout("loadTransantiagoPar();",1000);
			
		}else
		{
			deleteMarkerServicio();
			
			var capaContenedora = document.getElementById("cont_otro");
			
			capaContenedora.innerHTML=capaContenedora.innerHTML=CM_MSG_TRAN_ERROR;	
			
			var capaContenedora = document.getElementById("resultados_c");
			var msg=replaceAll(CM_MSG_TRANSZOOM,'_ZOOM_',''+map.getZoom()+'');
			var msg=replaceAll(msg,'_MIN_TRANS',''+MIN_ZOOM_TRANS+'');
			capaContenedora.innerHTML=capaContenedora.innerHTML=msg;	
	...
	deleteServicioMapaPago(100);
	if(map.getZoom() >= MIN_ZOOM_PAGO)
	{
		var AM_exten = getExtencion();
		$("#output").load("query_mapa.php", 
			{tipo:10, loni:AM_exten.left, lati:AM_exten.bottom, lond:AM_exten.right, lats:AM_exten.top} 
				,function(){	
				}
		);
	}
...
function addMarcadorOnDemandPago(CM_icono,CM_size,CM_lat,CM_lon,CM_texto,CM_id,CM_texto_mini)
{
		var icon_size_arr=CM_size.split(',');
	var size = new OpenLayers.Size(icon_size_arr[0],icon_size_arr[1]);	
  ...
  markers_ondemandPagos[CM_id].addMarker(marker_servicio);  
  markers_ondemandPagos[CM_id].setZIndex(zindex_marcador);
  //moverCentro(CM_lat,CM_lon,13);
}
...
function deleteServicioMapaPago(CM_id)
{
	
	try{
		
		  markers_ondemandPagos[CM_id].destroy();
		  markers_ondemandPagos[CM_id] = new OpenLayers.Layer.Markers( "puntos" );
    	map.addLayer(markers_ondemandPagos[CM_id]);    	
    	markers_ondemandPagos[CM_id].setZIndex(zindex_marcador);
    	$("#cont_res_"+CM_id).html("");
}catch(e){}
}
...
function checkServPago(CM_id_servicio_gis,CM_icono,CM_id)
{
	var elm=document.getElementById("img_serv_pago_"+CM_id);
	if(getEstadoServicioPago(CM_id)==0)
	{
		elm.className="serv_circulo_off";
		desactivarServicioPago(CM_id);
		deleteServicioMapaPago(CM_id);
	}else
	{
		if(getEstadoServicioPago(CM_id)==1)
		{
			elm.className="serv_circulo_on";
			activarServicioPago(CM_id);
			loadServEspPago(CM_id_servicio_gis,CM_icono,CM_id);
		}
	}
...
function loadServEspPago(CM_id_servicio_gis,CM_icono,CM_id)
{
	showLoad();
	//alert(String(getExtencion()));
	activarServicioPago(CM_id);
	...
	var capaContenedora = document.getElementById("resultados");
		...
		//capaContenedora.innerHTML=capaContenedora.innerHTML='<font class=AP_titulo2>Buscando...</font>';
		$("#output").load("query_mapa.php", 
			{tipo:9, loni:AM_exten.left, lati:AM_exten.bottom, lond:AM_exten.right, lats:AM_exten.top, id:CM_id_servicio_gis, icono: CM_icono, id_serv:CM_id} 
				,function(){	
					hideLoad();
				}
		);
...
function loadManzanas()
{
	...
	var scr = document.createElement("script");
	scr.setAttribute("type","text/javascript");
	scr.setAttribute("src","query_mapa.php?tipo=5&loni="+AM_exten.left+"&lati="+AM_exten.bottom+"&lond="+AM_exten.right+"&lats="+AM_exten.top+"");
	scr.setAttribute("id","scriptTemporal");
	body.appendChild(scr);
...
function accionManzana()
{
	...
	if(CM_manzana==false)
	{
		...
		if(map.getZoom()>=MIN_ZOOM_MANZANA)
		{
			...
			{
				//showAlerta("El zoom es muy alto, zoom minimo necesario es "+MIN_ZOOM_MANZANA+".",4000);
				showAlerta("Para poder activar el control debes acercarte mas al mapa. El zoom minimo es "+MIN_ZOOM_MANZANA+".",4000);
			}
			markers.setZIndex(zindex_marcador_direccion);
			markers_servicio.setZIndex(zindex_marcador);
	}else
...
function activaAntenas()
{
	CM_antena=true;
			var elm=document.getElementById("img_serv_01");
		elm.className="serv_circulo_on";

	deleteServicioMapa(100);
		loadAntenas();
}
*/
function showLoad()
{
	$('.cont_web').html("<img class=center_img src='images/load_central.gif'><div id=txt_load>Buscando ...<br></div>");
...
/*MAPA segundo 2*/
var selectControl,drag,sprintersLayer,map2,layer2;
var ptos_vector=[];
function init2(CM_div)
{
	...
	CM_div_mapa=CM_div;
	//var extent=new OpenLayers.Bounds(-13833619.268151,-7439463.1207699,-136103.80135396,387688.57454255);
	  map2 = new OpenLayers.Map(CM_div,{
	  	
	  controls: []});
	 
      layer2 = new OpenLayers.Layer.OSM();
    layer2.transitionEffect = "resize";
    map2.addLayer(layer2);
    //map2.addControl(new OpenLayers.Control.Attribution());
  
    map2.addControl(new OpenLayers.Control.Navigation({'handleRightClicks':true,'zoomWheelEnabled': true}));		
    map2.addControl(new OpenLayers.Control.MousePosition(new OpenLayers.Control.PanZoom()));
    ...
    map2.addLayer(sprintersLayer);
     
   	selectControl = new OpenLayers.Control.SelectFeature(
	  		sprintersLayer,
				{ highlightOnly:true, toggle: true, onSelect: selectPunto, onUnselect: unselectLinea }
		);
    map2.addControl(selectControl);
    selectControl.activate();
   
    drag=new OpenLayers.Control.DragFeature(sprintersLayer,{    
     'onComplete':moveDrag});
    map2.addControl(drag);
  
    map2.setCenter(new OpenLayers.LonLat(-74.8250402,10.9445785).transform(
        new OpenLayers.Projection("EPSG:4326"), // de WGS 1984
        new OpenLayers.Projection("EPSG:900913") // a Proyección Esférica Mercator
      ), 15);
}
